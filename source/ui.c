...
void updateQuadCountdownUI(int timer, int countdown, int duration, int atkNum) {
    ...
    memset32(&se_mem[BARS_SBB][(atkNum+1)*32], 0, 15);
    ...
    // set a row to green
    memset16(&se_mem[BARS_SBB][atkNum*32], 1, 30);
    if (timer > duration) { // in countdown
        for (int i = 0; i < 30; i++) se_mem[BARS_SBB][atkNum*32 + i] |= SE_PALBANK(15); // red (duration)
    }
    if (timer <= duration) { // in duration
        for (int i = 0; i < 30; i++) se_mem[BARS_SBB][atkNum*32 + i] |= SE_PALBANK(14); // green (countdown)
    }
    ...
    memset16(&se_mem[BARS_SBB][atkNum*32], 0, 30-t);
}
